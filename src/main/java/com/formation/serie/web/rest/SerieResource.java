...
/**
 * REST controller for managing {@link com.formation.serie.domain.Serie}.
 */
@RestController
@RequestMapping("/api")
@Transactional
    ...
    public ResponseEntity<Serie> partialUpdateSerie(@PathVariable(value = "id", required = false) final Long id, @RequestBody Serie serie)
        ...
        Optional<Serie> result = serieRepository
            ...
            .map(existingSerie -> {
                ...
                if (serie.getDateHeureAjout() != null) {
                    existingSerie.setDateHeureAjout(serie.getDateHeureAjout());
                }
